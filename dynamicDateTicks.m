function dynamicDateTicks(axH, link, axes2show, mdformat)
...
% dynamicDateTicks(axH, 'link', axes2link)
% 
...
if nargin < 3  %if no specification is given to which axes to link, link all
    axes2show = 'all';
end

if nargin < 4 % Default mm/dd format
    mdformat = 'mm/dd';
end
...
for i = 1:length(axH)
    ...
    addlistener(axH(i), 'YLim', 'PostSet', @(src, evt)changeYlabelCallback(src, evt));
...
set(figH,'SizeChangedFcn',@changeYlabelCallback);
...
% ------------ End of dynamicDateTicks-----------------------
    ...
    function updateDateLabel(obj, ev, varargin)
        ...
        if strcmpi(axes2show,'last')
            if size(axesInfo.Linked,2) > 1
           
                set(axesInfo.Linked(1:end-1), 'XTickLabel', []);
    ...
    function changeYlabel
        
        if ~isempty(axesInfo) 
       
            for k = 1 : numel(axesInfo.Linked)

                yLabelVal = get(axesInfo.Linked(k),'YTick');
                yLabelStr = cellstr(num2str(yLabelVal'));
                yLabelStr{end} = '';
                set(axesInfo.Linked(k),'YTickLabel',yLabelStr);

            end
        end
    end

    function changeYlabelCallback(src,evt)
        changeYlabel();
    end
